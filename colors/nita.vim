...
let g:colors_name = "nita"

" Check to see if we can do colors 8-15
if has('gui_running') || (&t_Co > 8 && (!exists('g:nita_nobright') || g:nita_nobright != 1))
	let s:gt_eight = 1
else
	let s:gt_eight = 0
endif
...
if &background == "dark" && s:gt_eight && (!exists('g:nita_fg_dark') || g:nita_fg_dark != 1)
...
if exists('g:nita_red_error_only') && g:nita_red_error_only == 1
	let s:truered = s:red
	let s:red     = s:dimcyan
...
if exists('g:nita_color_map')
	for [s:key, s:val] in items(g:nita_color_map)
...
if s:dimtwo != s:dim " eeds to be different from Comment
